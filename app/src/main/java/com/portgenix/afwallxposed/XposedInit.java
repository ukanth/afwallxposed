...
public class XposedInit implements IXposedHookZygoteInit, IXposedHookLoadPackage {
    ...
    private void reloadPreference() {
        try {
            ...
            if (prefs == null) {
                ...
                prefs.reload();
                sharedPreferences = new RemotePreferences(context, BuildConfig.APPLICATION_ID, PREFS_NAME);
            } else {
                prefs.makeWorldReadable();
                prefs.reload();
            }
            //pPrefs = context.getSharedPreferences(Api.PREFS_NAME,Context.MODE_PRIVATE);
            pPrefs = new SharePreference(context, MY_APP, PREFS_NAME);
    ...
    private void interceptDownloadManager(XC_LoadPackage.LoadPackageParam loadPackageParam) {
        ...
        XC_MethodHook dmSingleResult = new XC_MethodHook() {
            ...
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                reloadPreference();
                //TODO: get the app allowed from afwall ?
                boolean isAppAllowed = false;
                //final boolean isAppAllowed = isAppAllowed(context, applicationInfo, sharedPreferences, pPrefs);
